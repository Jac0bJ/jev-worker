...
import type { AppEnv, AppConfig, ClientIdentity, DecideRequest, DecisionOutcome, JevInput, JevResult, CacheStatus, DecisionResponse } from "./types";

export function createDecisionService(options: {
  env: AppEnv; config: AppConfig; provider: JevProvider; client: ClientIdentity;
  requestId: string; preset?: string;
}): (request: DecideRequest) => Promise<DecisionOutcome> {
  const { env, config, provider, client, requestId, preset } = options;
  return async (request) => {
    const start = performance.now();
    const input: JevInput = { state: request.state, questions: request.questions };
    ...
    try {
      ...
      if (request.cache !== false) {
        try {
          ...
          if (stored !== null) { result = parseProviderResult(stored, input); cache = "HIT"; }
        } catch { cache = "ERROR"; }
      }
      if (!result) {
        ...
        if (cacheKey) {
          try { await writeCache(env.JEV_CACHE, cacheKey, result, config.cacheTtlSeconds); }
          catch { cache = "ERROR"; }
        }
      }
      ...
      logDecision({
        requestId, ...(preset ? { preset } : {}), latencyMs: Math.round(performance.now() - start),
        cache, provider: provider.id, ...(result ? { model: result.model } : {}), status,
        ...(errorCode ? { errorCode } : {}),
        ...(data ? { answers: Object.values(data.answers).map(answer => ({ type: answer.type, confidence: answer.decision_confidence, decision: answer.decision, source: answer.decision_confidence_source })) } : {}),
      }, input, config.logInputs);
    }
  };
}
