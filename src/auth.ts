...
export async function authenticate(
  ...
  if (!config.authEnabled) {
    if (config.environment !== "development") {
      throw new AppError(
        503,
        "CONFIG_ERROR",
        "Authentication may only be disabled in development.",
      );
    }
  ...
  if (keys.length === 0) {
    throw new AppError(
      503,
      "CONFIG_ERROR",
      "Client API keys are not configured.",
    );
  }
  ...
  for (const configuredHash of configuredHashes) {
    matched =
      crypto.subtle.timingSafeEqual(suppliedHash, configuredHash) || matched;
  }
  if (!matched) {
    throw new AppError(401, "UNAUTHORIZED", "A valid API key is required.");
  }
