...
export async function evaluateDecision(
  ...
  try {
    ...
    if (controller.signal.aborted)
      throw new AppError(504, "UPSTREAM_TIMEOUT", "Jev request timed out.");
    throw error;
  } finally {
...
function decision(confidence: number, config: AppConfig): Decision {
  return confidence >= config.confidenceAuto
    ? "auto"
    : confidence >= config.confidenceReview
      ? "review"
      : "reject";
}
export function routeResult(
  ...
  for (const [id, answer] of Object.entries(result.answers)) {
    ...
    answers[id] = {
      ...
      decision_confidence_source:
        answer.type === "noul" ? "derived-noul" : "model",
    };
  }
