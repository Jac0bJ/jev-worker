...
export function createTypeSafeProvider(
  ...
  return {
    ...
    async evaluate(input, { signal }) {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403)
          throw new AppError(
            503,
            "UPSTREAM_AUTH_ERROR",
            "Jev credentials were rejected.",
          );
        if (response.status === 429)
          throw new AppError(
            503,
            "UPSTREAM_RATE_LIMITED",
            "Jev rate limit was reached.",
          );
      ...
      try {
        return parseProviderResult(await boundedJson(response), input);
      } catch (error) {
        if (signal.aborted || error instanceof AppError) throw error;
        throw new AppError(
          502,
          "UPSTREAM_ERROR",
          "Jev response could not be read.",
        );
      }
    },
  };
