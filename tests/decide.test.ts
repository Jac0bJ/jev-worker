...
describe("routing", () => {
  it("derives noul confidence for strong yes and no, with inclusive thresholds", () => {
    const source = { ...sampleResult, answers: { yes: { type: "noul" as const, noul: .9 }, no: { type: "noul" as const, noul: .1 }, uncertain: { type: "noul" as const, noul: .5 } } };
  ...
  it("uses model confidence for choice and score", () => {
    const routed = routeResult({ ...sampleResult, answers: { c: { type: "choice", choice: "a", confidence: .6, probabilities: { a: 1 } }, s: { type: "score", score: 1.2, confidence: .59, probabilities: { "0": .2, "1": .6, "2": .2 }, legend: { "0": "Low", "1": "Mid", "2": "High" } } } }, config);
...
describe("provider evaluation", () => {
  it("passes a signal and returns the provider result", async () => {
    const evaluate = vi.fn(async (_input: typeof sampleInput, _options: { signal: AbortSignal }) => { void _input; void _options; return sampleResult; });
    const provider: JevProvider = { id: "workers-ai", model: "typesafe/jev", evaluate };
    await expect(evaluateDecision(sampleInput, provider, config)).resolves.toEqual(sampleResult);
  ...
  it("aborts and fails when the provider ignores cancellation", async () => {
    ...
    const provider: JevProvider = { id: "workers-ai", model: "typesafe/jev", evaluate: async (_, { signal }) => { signalSeen.push(signal); return new Promise(() => undefined); } };
    try {
      const pending = evaluateDecision(sampleInput, provider, { ...config, upstreamTimeoutMs: 10 });
      const assertion = expect(pending).rejects.toMatchObject({ status: 504, code: "UPSTREAM_TIMEOUT" });
