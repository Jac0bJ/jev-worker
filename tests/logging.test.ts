...
const input: JevInput = { state: "private user message", questions: { secretQuestion: { type: "noul", instructions: "private rubric" } } };
const event: DecisionLog = { requestId: "request-1", latencyMs: 9, cache: "MISS", provider: "workers-ai", status: 200, answers: [{ type: "noul", confidence: 0.9, decision: "auto", source: "derived-noul" }] };
...
describe("logDecision", () => {
  it("emits one structured line without user content or question IDs by default", () => {
    ...
    expect(JSON.parse(line)).toMatchObject({ requestId: "request-1", status: 200, answers: [{ type: "noul", decision: "auto" }] });
