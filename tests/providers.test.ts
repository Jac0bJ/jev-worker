...
describe("TypeSafe HTTP adapter", () => {
  it("posts only the documented payload and validates the answer", async () => {
    ...
    await expect(provider.evaluate(sampleInput, { signal })).resolves.toEqual(
      sampleResult,
    );
  ...
  it("rejects oversized, malformed and failed upstream responses", async () => {
    const oversized = createTypeSafeProvider(
      "key",
      vi.fn(
        async () =>
          new Response("x", { headers: { "content-length": "2097153" } }),
      ) as typeof fetch,
    );
    await expect(
      oversized.evaluate(sampleInput, { signal }),
    ).rejects.toMatchObject({ code: "UPSTREAM_INVALID_RESPONSE" });
    const failed = createTypeSafeProvider(
      "key",
      vi.fn(
    ...
    await expect(
      failed.evaluate(sampleInput, { signal }),
    ).rejects.toMatchObject({ code: "UPSTREAM_AUTH_ERROR" });
  });
  it.each([
    ...
    async (status, expectedStatus, code) => {
      ...
      await expect(
        provider.evaluate(sampleInput, { signal }),
...
describe("Workers AI adapter", () => {
  ...
  it("requires the configured provider prerequisite", () => {
    ...
    expect(() =>
      createProvider({} as AppEnv, { ...config, provider: "typesafe" }),
    ).toThrow();
  });
});
