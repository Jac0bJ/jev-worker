...
const input: JevInput = { state: { z: "text", a: { y: 2, x: 1 } }, questions: { spam: { type: "noul", instructions: "Is this spam?" } } };
const provider = { id: "workers-ai", model: "typesafe/jev" } as JevProvider;
const result: JevResult = { model: "typesafe/jev", answers: { spam: { type: "noul", noul: 0.9 } }, usage: { input_tokens: 4, output_tokens: 1 } };
...
describe("cache", () => {
  it("canonicalizes object keys and isolates client, provider, and model", async () => {
    ...
    const reordered: JevInput = { questions: input.questions, state: { a: { x: 1, y: 2 }, z: "text" } };
    expect(await makeCacheKey(reordered, provider, { fingerprint: "one" })).toBe(key);
    expect(await makeCacheKey(input, provider, { fingerprint: "two" })).not.toBe(key);
    expect(await makeCacheKey(input, { ...provider, id: "typesafe" }, { fingerprint: "one" })).not.toBe(key);
    expect(await makeCacheKey(input, { ...provider, model: "next" }, { fingerprint: "one" })).not.toBe(key);
