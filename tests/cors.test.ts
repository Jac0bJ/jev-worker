...
const request = (origin?: string, method = "POST") =>
  new Request("https://api.example/decide", {
    method,
...
describe("CORS", () => {
  ...
  it("rejects disallowed origins but permits no Origin", () => {
    expect(() =>
      corsHeaders(request("https://evil.example"), config),
    ).toThrowError(
      expect.objectContaining({ code: "CORS_FORBIDDEN", status: 403 }),
    );
    expect(
      corsHeaders(request(), config).get("Access-Control-Allow-Origin"),
    ).toBeNull();
  });
  it("handles preflight before auth and supports opt-in wildcard", () => {
    const response = handlePreflight(
      request("https://app.example", "OPTIONS"),
      config,
    );
    ...
    expect(
      corsHeaders(request("https://other.example"), {
        ...config,
        allowedOrigins: ["*"],
