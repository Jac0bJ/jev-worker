...
describe("presets", () => {
  it("exposes the promised presets and question IDs", () => {
    expect(Object.keys(presets).sort()).toEqual(["comment-moderation", "form-spam", "lead-quality", "support-route"]);
    expect(Object.keys(presets["form-spam"].questions)).toEqual(["spam"]);
    expect(Object.keys(presets["comment-moderation"].questions)).toEqual(["category", "severity"]);
    expect(Object.keys(presets["lead-quality"].questions)).toEqual(["quality"]);
    expect(Object.keys(presets["support-route"].questions)).toEqual(["department"]);
    ...
    if (moderation.category.type === "choice") {
      expect(Object.keys(moderation.category.criteria)).toEqual(["ok", "spam", "harassment", "hate", "sexual", "self-harm"]);
    }
    expect(moderation.severity.type).toBe("score");
    if (moderation.severity.type === "score") expect(moderation.severity.criteria).toHaveLength(5);
    ...
    if (route.type === "choice") expect(Object.keys(route.criteria)).toEqual(["billing", "bug", "feature-request", "account", "other"]);
  });
  ...
  it("resolves a preset into a decision request and validates its body", () => {
    ...
    expect(result.input).toEqual({ state: "Hello", questions: presets["form-spam"].questions });
    expect(() => resolvePreset("form-spam", { text: " " })).toThrowError(AppError);
    expect(() => resolvePreset("form-spam", { text: "Hello", cache: false })).toThrowError(AppError);
  });
  ...
  it("adds fractional lead rating without modifying the routed or cached answer", () => {
    const answer = {
      type: "score" as const, score: 2.25, confidence: .94,
      probabilities: { "0": 0, "1": 0, "2": .75, "3": .25, "4": 0 },
      legend: { "0": "Low", "1": "Below", "2": "Medium", "3": "High", "4": "Best" },
      decision: "auto" as const, decision_confidence: .94, decision_confidence_source: "model" as const,
    };
    const response: DecisionResponse = { model: "jev", answers: { quality: answer }, usage: { input_tokens: 2, output_tokens: 1 } };
